...
def direct_recharge():
    ...
    try:
        ...
        try:
            ...
            if amount_int < 10 or amount_int > 500:  # MAX_CHARGE_500 [TEMP:10]
...
def create_recharge_link():
    ...
    try:
        ...
        try:
            ...
            if amount_int < 10 or amount_int > _link_max:  # MAX_LINK_PER_MERCHANT [TEMP:10]
                return jsonify({'success': False, 'error': f'المبلغ يجب أن يكون بين 10 و {_link_max} ريال'})
        except (ValueError, TypeError):
            return jsonify({'success': False, 'error': 'المبلغ يجب أن يكون رقماً'})
