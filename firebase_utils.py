...
def get_category_gift_title(category_id):
    """اسم/عنوان اول هدية متاحة لقسم معين (للعرض للعميل بدون كشف السري)."""
    ...
    try:
        for gd in db.collection('category_gifts').stream():
            ...
            if str(g.get('category_id', '')) == str(category_id) and not g.get('used', False):
                return g.get('gift_title', '') or ''
        return ''
    except Exception as e:
        logger.error(f"get_category_gift_title error: {e}")
        return ''
...
def peek_category_gift(user_id, category_id, category_name, goal=10):
    """يتحقق من الاهلية ويرجع بيانات الهدية بدون سحبها (للهدايا اليدوية)."""
    if not db or not user_id or not category_id:
        return (False, 'بيانات ناقصة')
    try:
        gift_data = None
        ...
        try:
            logger.error(f"peek_category_gift error: {e}")
        except Exception:
            pass
